import { CONNECT_DOTS_CONFIG, type ConnectDotsDifficulty } from "@shared/game/connect-dots";
...
import { validatePollQuestions } from "@shared/game/polls";
import { validatePieceUnlockAt, resolvePieceUnlockAt } from "@shared/game/jigsaw-tile-rewards";
import type {
  GamePayload,
  QuizOptionId,
  QuizQuestionDraft,
...
import { isTimerMode, isTimerValid, resolvePayloadTimerMode } from "@shared/game/timer";
...
export function validateGamePayload(
  ...
  if (payload.timerMode !== undefined && !isTimerMode(payload.timerMode)) {
    return { ok: false, error: "Choose either a whole-game or per-question timer." };
  }
  const timerMode = resolvePayloadTimerMode(payload);
  if (!isTimerValid(mode, payload.timeLimitSeconds, timerMode)) {
    return {
      ok: false,
      error:
        timerMode === "per_question"
          ? "Choose 5 seconds to 5 minutes per question, or no limit."
          : "Choose a valid whole-game timer, or no limit.",
    };
  }
