import { buildConnectDotsFromContentPairs } from "@shared/game/connect-dots-content";
...
import { rankConnectDots, rankJigsaw, rankQuiz } from "@shared/game/ranking";
import type { ConnectDotsBoardConfig, GamePayload, LeaderboardRow, Room } from "@shared/game/types";
import {
  toPublicQuizQuestions,
  toPublicVisualPointQuestions,
} from "@shared/game/validation";
import { normalizeVisualPointQuestion } from "@shared/game/visual-point";
import type { StoredRoom } from "@shared/game/server-types";
import { CORE_LIVE_GAME_MODES, type CoreLiveGameMode } from "@shared/game/session-flow";
import { normalizePollPayload } from "@shared/game/polls";
import { resolvePayloadTimerMode } from "@shared/game/timer";
...
const QUIZ_MODE: LiveModeDefinition = {
  ...
  normalizeCreatePayload: (payload) => {
    ...
    return {
      ...
      timerMode: resolvePayloadTimerMode(payload),
    };
  },
...
const JIGSAW_MODE: LiveModeDefinition = {
  ...
  normalizeCreatePayload: (payload) => {
    ...
    const pieceUnlockAt = defaultPieceUnlockAt(payload.questions.length, tileCount);
    return {
      ...payload,
      jigsaw: {
        ...payload.jigsaw,
