...
export function SessionFilesPanel({ roomId, authorToken, className }: SessionFilesPanelProps) {
  ...
  const upload = async () => {
    ...
    setError(null);
    try {
      const next = await uploadSessionFiles(roomId, authorToken, stagedFiles);
      setSummary(next);
      setStagedFiles([]);
      toast.success("Files ready for students.");
    } catch (err) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              One QR opens all active documents for this session.
            </p>
          </div>
            ...
            <div className="grid gap-2">
              {summary.files.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-[var(--gamibar-border)] bg-[var(--gamibar-page)] px-4 py-5 text-center text-sm text-[var(--muted-foreground)]">
                  Add a document and students will see it on the QR page.
                </div>
              )}
            </div>
          </div>
