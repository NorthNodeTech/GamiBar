    ...
    >
      <div className="border-b border-[var(--gamibar-border)] bg-[var(--gamibar-surface)] px-5 py-4 sm:px-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--muted-foreground)]">Top performers update in real time</p>
          </div>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        <div className="px-6 py-14 text-center">
          ...
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">Rankings appear once students start playing.</p>
        </div>
      ) : (
        <>
          {topThree.length > 0 && (
            ...
            <ol className="max-h-72 space-y-1 overflow-y-auto p-3 sm:p-4">
              {rest.map((row) => (
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-[var(--muted-foreground)]">{row.detail}</span>
                  {row.secondaryMetric != null && (
                    <span className="text-xs text-[var(--muted-foreground)]">{formatDuration(row.secondaryMetric)}</span>
                  )}
                </li>
              ))}
            </ol>
...
function PodiumSpot({
      ...
      >
        <span
          className={cn(
            ...
            rank === 3 && "bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300",
          )}
        >
          {rank === 1 ? "🥇" : rank === 2 ? "🥈" : "🥉"}
        </span>
        ...
        {row.secondaryMetric != null && (
          <span className="shrink-0 text-xs text-[var(--muted-foreground)]">{formatDuration(row.secondaryMetric)}</span>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <div className="relative mb-2">
        ...
        <span
          className={cn(
            ...
            rank === 1 && "size-12 bg-amber-100 text-amber-900 dark:bg-amber-500/20 dark:text-amber-300",
            rank === 2 && "bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-300",
            rank === 3 && "bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300",
            highlighted &&
      ...
      <p className="line-clamp-1 max-w-[88px] text-xs font-semibold text-[var(--foreground)]">{row.displayName}</p>
...
export function CompletionTimeline({
  ...
  return (
    ...
    >
      ...
      <p className="mt-0.5 text-xs text-[var(--muted-foreground)]">Latest finishes across the room</p>
            ...
            <li key={item.key} className="relative flex gap-4 pb-6 last:pb-0">
              ...
              <div className="min-w-0 flex-1 rounded-xl border border-[var(--gamibar-border)] bg-[var(--gamibar-page)] px-3 py-2.5">
                <p className="text-sm text-[var(--muted-foreground)]">
                  <span className="font-semibold text-[var(--foreground)]">{item.displayName}</span> completed
