...
import { GAME_CONFIG, GAME_MODE_META, JIGSAW_GRID, PUZZLE_QUEST_GRID } from "@shared/game/config";
import { buildGameCompletionViewModel } from "@shared/game/completion";
...
import { isStudentSessionFinished } from "@shared/game/mode-registry";
...
import { readPollResponses, type PollsPayload } from "@shared/game/polls";
import type {
...
import type { PathMap } from "@shared/game/connect-dots";
...
export default function StudentPlayPage() {
  ...
  const handleQuestionTimerExpired = useCallback(
    async (stepId: string) => {
      ...
      expiringStepRef.current = stepId;
      try {
        const result = await expireQuestionTimerFn({
          data: { roomId, reconnectToken, stepId },
        });
        if (!result.ok && result.error !== "This question still has time remaining.") {
          toast.error(friendlyGameError(result.error, "Could not advance the question timer."));
        }
        await refresh();
      } catch {
        toast.error("Could not advance the timed question. Reconnecting...");
        await refresh();
      } finally {
        expiringStepRef.current = null;
      }
    },
    [reconnectToken, refresh, roomId],
  );
  ...
  if (room.mode === "quiz" && room.payload.mode === "quiz") {
    return playShell(
      <QuizPlay
        ...
        answeredIds={
          new Set([
            ...snapshot.myAnswers.map((a) => a.questionId),
            ...snapshot.myTimedOutQuestionIds,
          ])
        }
    ...
    return playShell(
      <PollPlay
        ...
        currentQuestionIndex={
          typeof snapshot.myAttempt?.payload?.pollQuestionIndex === "number"
            ? snapshot.myAttempt.payload.pollQuestionIndex
            : 0
        }
        onTimerExpired={onActiveTimerExpired}
...
function TimerBar({
  ...
  return (
    ...
    >
      {left === 0 ? `${label} time's up` : `${label} · ${left}s`}
    </span>
  );
}
...
function QuizPlay({
  ...
  useEffect(() => {
    ...
    setActiveTimerStepId(timerStepId);
  }, [endsAt, timerStepId]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (timerMode === "per_question") {
        ...
        setActiveTimerStepId(res.questionTimer?.stepId ?? null);
      }
...
function PollPlay({
  ...
  const currentRequiredMissing = Boolean(
    currentQuestion?.required && !hasPollValue(responses[currentQuestion.id]),
  );
  ...
  const submitAll = async () => {
    ...
    try {
      const res = await submitPollResponsesFn({
        data: { roomId, reconnectToken, responses },
      });
      if (!res.ok) {
  ...
  const submitCurrent = async () => {
    if (!currentQuestion || submitting || timedOut) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      const res = await submitPollQuestionResponseFn({
        data: {
          roomId,
          reconnectToken,
      ...
      if (!res.ok) {
        const message = friendlyGameError(res.error, "Could not save this response.");
        setSubmitError(message);
        toast.error(message);
        return;
      }
      setTimerEndsAt(res.questionTimer?.endsAt ?? null);
      setActiveTimerStepId(res.questionTimer?.stepId ?? null);
      if (res.completed) toast.success("Response submitted");
      onSubmitted();
    } catch {
      const message = "Could not save this response. Check your connection and try again.";
      setSubmitError(message);
      toast.error(message);
    } finally {
      setSubmitting(false);
  ...
  const visibleQuestions =
    timerMode === "per_question" && currentQuestion ? [currentQuestion] : payload.questions;
  const blocked =
    (timerMode === "per_question" ? currentRequiredMissing : requiredMissing) ||
    submitting ||
    timedOut;
    ...
    <div className="mx-auto flex min-h-dvh-screen w-full max-w-5xl flex-col px-4 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:px-5 sm:py-6 lg:px-8 lg:py-8">
      ...
      {timedOut ? (
        <p className="mt-4 rounded-xl bg-red-50 px-3 py-2 text-center text-sm font-medium text-red-800">
          {timerMode === "per_question"
            ? "Question time expired - moving to the next item..."
      ...
      <div className="mt-5 grid gap-4">
        {visibleQuestions.map((question) => {
          const index = payload.questions.findIndex((candidate) => candidate.id === question.id);
          return (
            <PollQuestionCard
              key={question.id}
              question={question}
      ...
      <div className="sticky bottom-0 mt-6 border-t border-[var(--gamibar-border)] bg-white/95 py-3 backdrop-blur supports-[backdrop-filter]:bg-white/80">
        <Button
          ...
          disabled={blocked}
          onClick={() => void (timerMode === "per_question" ? submitCurrent() : submitAll())}
        >
          {submitting
            ? "Saving..."
            : timerMode === "per_question" && currentQuestionIndex < payload.questions.length - 1
              ? "Save and continue"
              : "Submit response"}
        </Button>
        {(timerMode === "per_question" ? currentRequiredMissing : requiredMissing) && !timedOut ? (
        ...
        {submitError ? (
          <InlineErrorBanner
            ...
            message={submitError}
            onRetry={() => void (timerMode === "per_question" ? submitCurrent() : submitAll())}
            retrying={submitting}
            onDismiss={() => setSubmitError(null)}
          />
        ) : null}
...
function VisualPointPlay({
  ...
  const firstUnansweredIndex = questions.findIndex(
    (question) => !localAnsweredIds.has(question.id),
  );
  const maxViewIndex =
    firstUnansweredIndex >= 0 ? firstUnansweredIndex : Math.max(0, questions.length - 1);
  const [viewIndex, setViewIndex] = useState(() => maxViewIndex);
  ...
  return (
    <div className="mx-auto flex min-h-dvh-screen w-full max-w-5xl flex-col px-4 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:px-5 sm:py-6 lg:px-8 lg:py-8">
      ...
      {timedOut && isActive ? (
        ...
        >
          {timerMode === "per_question"
            ? "Question time expired - moving to the next image..."
            : "Time's up - no more answers can be submitted."}
        </p>
      ) : null}
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-[auto_1fr_auto] sm:items-center">
        ...
        {isAnswered ? (
          <p className="text-center text-sm font-medium text-[var(--muted-foreground)]">
            {timedOutIds.has(current.id)
              ? "This image was skipped when its timer expired."
...
function ConnectDotsGridPlay({
  ...
  const syncProgress = useCallback(
    ...
    [reconnectToken, roomId, timerMode],
  );
      ...
      <div className="mx-auto w-full max-w-lg px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:max-w-xl sm:px-5 sm:py-6">
        <div className="sticky top-0 z-10 -mx-4 mb-4 border-b border-[var(--gamibar-border)]/80 bg-[var(--gamibar-page)]/95 px-4 py-3 backdrop-blur-sm sm:-mx-5 sm:px-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <TimerBar
              endsAt={timerEndsAt}
              label={timerMode === "per_question" ? "Pair" : "Game"}
              onTimedOut={setTimedOut}
              onExpired={() => onTimerExpired?.(activeTimerStepId ?? undefined)}
            />
          </div>
          ...
          {timedOut && !done ? (
            <p className="mt-2 text-xs font-semibold text-red-700" role="status">
              {timerMode === "per_question"
                ? "Pair time expired - a fresh countdown is starting..."
...
function JigsawMissionPlay({
  ...
  const unlockSchedule = useMemo(() => resolvePieceUnlockAt(total, tileCount), [total, tileCount]);
  ...
  const pendingPiece = useMemo(() => {
    const progress = activeTileQuestionProgress(correctSet.size, total, tileCount);
    if (!progress) return null;
    return { ...progress, tileId: tileIdFromIndex(progress.pieceIndex, cols) };
  }, [correctSet.size, total, tileCount, cols]);
  ...
  const [displayedTileIds, setDisplayedTileIds] = useState<string[]>(() =>
    readEarnedTileIds(
      ...
      total,
      resolvePieceUnlockAt(total, tileCount),
    ),
  );
  ...
  return (
    <div className="flex min-h-dvh-screen min-w-0 flex-col overflow-x-hidden bg-[#FAFAFA]">
      <SlidoProgressHeader
        ...
        onTimedOut={setTimedOut}
        onExpired={() => onTimerExpired?.(activeTimerStepId ?? undefined)}
        timerLabel={timerMode === "per_question" ? "Question" : "Game"}
      />
          ...
          >
            <div className="order-1 min-w-0 rounded-2xl border border-[#E5E7EB] bg-white p-2.5 shadow-sm sm:p-3 md:p-6">
              ...
              <p className="mt-2 text-center text-[10px] leading-snug text-[#737373] md:mt-4 md:text-xs">
                {isRetryRound
                  ...
                  : `${tilesUnlocked}/${tileCount} pieces fully colored · answer correctly to fill the next piece`}
              </p>
            </div>
            ...
            <div className="order-2 min-w-0 rounded-2xl border border-[#E5E7EB] bg-white p-3 shadow-sm sm:p-4 md:p-6">
              ...
              <SlidoQuizPanel
                ...
                correctFeedback="Correct! The current piece gained more color."
                wrongFeedback="Not quite — no color added yet. This question will return later."
  ...
  return (
    <div className="flex min-h-dvh-screen flex-col bg-[#FAFAFA]">
      ...
      {timedOut ? (
        ...
        >
          {timerMode === "per_question"
            ? "Question time expired - retrying with a fresh countdown..."
            : "Time's up - no more answers can be submitted."}
        </p>
      ) : null}
